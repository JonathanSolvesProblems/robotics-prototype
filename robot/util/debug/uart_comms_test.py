...
# default port on odroid for UART
port = "/dev/ttySAC0"
#port = "/dev/ttyACM0"
baudrate = 115200
msg = "who\n"
...
if len(sys.argv) <= 3:
    ...
    print("If you've done it before then try running the following command to remember:")
    print("history | grep test.py")
    sys.exit(0)
elif len(sys.argv) == 4:
    '''
    if sys.argv[1] == "who":
...
while True:
    ...
    while ser.in_waiting:
        print("response1: ", ser.readline().decode())

    time.sleep(0.7)

    while ser.in_waiting:
        print("response2: ", ser.readline().decode())
